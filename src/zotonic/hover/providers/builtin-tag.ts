...
export class BuiltinTagHoverProvider extends DocHoverProvider {
    public host: string;

    constructor({ host }: ConstructorArgs) {
        super({
            pattern:
                /(?<={%\s*(optional\s*)?)(all\s+catinclude|all\s+include|autoescape|block|cache|call|catinclude|comment|cycle|extends|filter|firstof|for|if|if d|ifchanged|ifequal|ifnotequal|image|image_data_url|image_url|include|inherit|javascript|javascript|lib|load|media|now|overrules|print|raw|regroup|spaceless|templatetag|trans|trans_ext|url|with)/,
            genUrl: (tag) => {
